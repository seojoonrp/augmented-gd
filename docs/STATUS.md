...
| **Blunt** (hazard hitboxes 80/60/40 %) | New 2026-09-16, untested. `HazardHitboxHook.cpp` hooks `GameObject::getObjectRect` (AABB, in place when freshly recomputed) + `updateOrientedBox` (OBB corners toward centre, qolmod pattern); `m_objectRadius` scaled in `PlayLayer::addObject` / `applyBlunt`. Debug key **6**. | Pattern from qolmod AccurateHitboxes (proves OBB corner edits change collision). Whether `getObjectRect()` is the only filler of `m_objectRect` is unverified. | Start a run, press 6 (Lv1) with Foresight (4) on: red boxes must visibly shrink; fly the ship/wave through a spike tip that kills without Blunt. Log must show `Blunt: hazard hitbox scale -> 0.80`, `Blunt: rect of object id …`, and on the next death `Blunt: shrunk N rects, M oriented boxes …` with N > 0. N == 0 → GD fills `m_objectRect` elsewhere → shrink eagerly in `addObject` instead. A `[returned ref is NOT m_objectRect]` marker → return `m_objectRect` from the hook. Saw test: a saw (circle) must also shrink. Rotated (e.g. 45°) spike: M > 0 and smaller box. |
...
- `Blunt: hazard hitbox scale -> …` / `Blunt: scale … applied to N hazards (M circular)` — keep
- `Blunt: rect of object id …` / `Blunt: oriented box of object id …` (first 4 after each scale change) and `Blunt: shrunk N rects, M oriented boxes since the last reset (…)` per reset — trim once verified
