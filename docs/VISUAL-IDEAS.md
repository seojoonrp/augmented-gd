...
### 2.1 게이지 주스 — 최우선 (부분 구현 2026-09-17: 바 + `charge/cost` 숫자, 이징. 떠오르는 `+N`·효과음·DRAFT READY는 아직)
...
### 2.2 진행바 장식 — 런의 역사가 보이는 곳 (베스트·체크포인트 점 구현 2026-09-17, `ProgressMarks`; 죽음 눈금은 아직)
...
### 2.4 HUD 아이콘 줄 (텍스트 줄 → 아이콘) (틀 구현 2026-09-17: 빈 아이콘 박스 + 이름 + 상태 줄, 맨 왼쪽; 아이콘 아트·활성 상태 표현은 아직)
