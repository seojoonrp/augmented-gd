...
|---|---|---|---|
| **Key input, Geode 5** — global listener | [qolmod](qolmod.md) `src/Keybinds/Hooks.cpp:51-96` | `$execute { KeyboardInputEvent().listen(+[](KeyboardInputData const&){…}).leak(); }` plus `listenForKeybindSettingPresses("open-menu-keybind", …)` in the same block | `PlayLayerHook.cpp` `$execute` (identical pattern, not firing → see [custom-keybinds](custom-keybinds.md) "Open problem") |
| Key input — node-scoped listener (auto-removed with the node) | [custom-keybinds](custom-keybinds.md) `src/UILayer.cpp:47-102` (PauseLayer), `:321-329` (UILayer) | `this->addEventListener(KeybindSettingPressedEventV3(Mod::get(), "id"), [this](Keybind const&, bool down, bool repeat, double ts){…})` | not used yet — preferred fix for hotkeys |
| Key input — who eats what | loader `platform/windows/input.cpp:492-520`, `loader/LoaderImpl.cpp:409-455`, `include/Geode/loader/Event.hpp:1045-1058` | order: key-filtered listeners → unfiltered listeners → IME → `CCKeyboardDispatcher`; loader's keybind listener only `Stop`s if a `KeybindSettingPressedEventV3` listener returned `Stop`; custom-keybinds swallows everything in `UILayer::handleKeypress` except Escape | — |
