...
|---|---|---|---|
| **Key input, Geode 5** — global listener | [qolmod](qolmod.md) `src/Keybinds/Hooks.cpp:51-96` | `$on_mod(Loaded) { KeyboardInputEvent().listen(fn, priority).leak(); }` — pass a **negative priority** to run before the loader's keybind listener | `src/input/Hotkeys.cpp` `$on_mod(Loaded)`, priority -1 |
...
| **Mirror portals** | [qolmod](qolmod.md) `src/Hacks/Level/NoMirrorPortal.cpp:22-31` | hook `GJBaseGameLayer::toggleFlipped(bool flip, bool noEffects)` and return early (`win 0x2467d0`) | `Unmirror.cpp` `toggleFlipped` hook (2026-09-17, unverified in game) |
...
| In-level HUD / labels | [qolmod](qolmod.md) `src/Labels/Hooks.cpp:9` | container added to `m_uiLayer` (screen space, survives camera) | `RunHud` in `m_uiLayer` **(verified)** |
| **Progress bar / top UI nodes missing after `PlayLayer::init`** | [node-ids](node-ids.md) `src/PlayLayer.cpp:69-110` | online levels pass `dontCreateObjects`; GD creates `m_progressBar`, percent label etc. in `setupHasCompleted` -> hook that too | `attachToProgressBar` from both hooks **(verified)**; geometry in `GD-INTERNALS.md` |
